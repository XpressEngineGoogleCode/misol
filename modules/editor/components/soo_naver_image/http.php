...
if(!defined("__ZBXE__")) exit(); // only this line is added by misol.
	...
	class http
	{
		...
		function dumpAll()
		{
			$data = "<div style='text-align: justify; margin-bottom: 30px; font-size: 9pt; font-family: ����; line-height: 150%; word-break: break-all'><fieldset><legend>&nbsp;���&nbsp;</legend>";

			foreach($this->receive as $header => $value)
			{
				if( $header == "Body" )
					$body = "<fieldset><legend>&nbsp;����&nbsp;</legend>".htmlspecialchars($value)."</fieldset>";
				else
					$data .= "$header: ".htmlspecialchars($value)."<br>";
			}

			echo "$data</fieldset><fieldset><legend>&nbsp;��Ű&nbsp;</legend>";
