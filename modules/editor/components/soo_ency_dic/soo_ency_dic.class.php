...
class soo_ency_dic extends EditorHandler {
     ...
     **/
    function soo_ency_dic($editor_sequence, $component_path) {
    ...
    function soo_search() {
        ...
        $soo_display_set=trim($this->soo_ency_display);
        ...
        $query_string = sprintf('/search?key=%s&target=encyc&start=%s&display=%s&query=%s', $naver_api_key, $soo_result_start, $soo_display_set, $query);
        ...
        $soo_result_start_end = trim($soo_result_start.' - '.($soo_result_start+$soo_search_display-1));
        if($total_result_no >= $soo_result_start+$soo_search_display && $soo_result_start+$soo_search_display <= '1000') {
        $soo_next_page=$soo_result_start+$soo_search_display;
        }
        else {
        $soo_next_page="1";
        }
        if($soo_result_start!='1') {
        $soo_before_page=$soo_result_start-$soo_search_display_set;
        }
        else {
        $soo_before_page="1";
        }
        ...
        for($i=0;$i<$soo_results_count;$i++) {
            ...
            $soo_list[] = sprintf("%s,soo,%s,soo,%s,soo,%s",
                ...
                trim($item->link->body),
                trim(str_replace(array('<b>','</b>','"'),array('','','&quot;'),$item->description->body)),
