...
function insertSooCom(i, j) {
    ...
    if(j != 1) {
        if(item[i][2]) {
          text += "<img style=\"float: left\" alt=\""+item[i][0]+"\" src=\""+item[i][2]+"\" \/>";
        }
        if(item[i][3] != 0 && item[i][3]) {
          text += "<br />"+soo_msg_lowprice+" : "+item[i][3]+soo_msg_pricewon;
        }
        if(item[i][4] != 0 && item[i][4]) {
          text += "<br />"+soo_msg_highprice+" : "+item[i][4]+soo_msg_pricewon;
        }
    }
...
function soo_search(start_page) {
  ...
  params['component'] = "soo_naver_shop";
...
function complete_search(ret_obj, response_tags, selected_image) {
  ...
  if(!total_result_no || total_result_no==0) html = no_result;
      ...
      xInnerHtml(list_zone, 'result loading...');
      for(var i=0;i<result_list.length;i++) {
        ...
        if(item[i][2]) {
          html += "<img class=\"result_images\" onclick=\"insertSooCom('"+i+"', 0);\" alt=\""+item[i][0]+"\" src=\""+item[i][2]+"\" \/>";
        }
        if(item[i][3] != 0 && item[i][3]) {
          html += "<br />"+soo_msg_lowprice+" : "+item[i][3]+soo_msg_pricewon;
        }
        if(item[i][4] != 0 && item[i][4]) {
          html += "<br />"+soo_msg_highprice+" : "+item[i][4]+soo_msg_pricewon;
        }
          html += "<br /></p><p align=\"right\"><a href=\"javascript:insertSooCom('"+i+"', 0);\"><strong>"+soo_msg_insult+"</strong></a> <a href=\"javascript:insertSooCom('"+i+"', 1);\"><strong>"+soo_msg_insult_linkonly+"</strong></a></p></div>";
      }
  }
  var nxtpg = "";
