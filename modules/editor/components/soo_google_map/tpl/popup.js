...
function soo_save_location(i,j) { //위치 정보를 배열로 저장
	...
	if(j!=1) {
		...
		saved_location[i][5] = jQuery("#ment").val();
	}
...
function soo_load_location() { //위치 정보를 배열에서 로드
	...
	jQuery("#ment").val(saved_location[i][5]);
...
function view_list() { //검색된 위치 정보를 배열에서 리스트로 뿌림
	...
	for(var i=0;i<result_array.length;i++) {
		...
		html += "<li class=\"result_lists\"><a href=\"javascript:map_point('"+i+"');\">"+result_array[i].formatted_address +"</a>";
		if(typeof(result_array[i].from)=="undefined") html += " by Google</li>";
		else html += " by "+result_array[i].from+"</li>";
	}
	if(result_from == 'naver') { html += "<li class=\"result_lists\"><p>Address result powered by<br />- <strong>Google API</strong><br />- <strong>Yahoo Maps</strong><br />- <img src=\"http://openapi.naver.com/logo/logo07_1.gif\" alt=\"NAVER OpenAPI\" /></p></li>"; }
	else if(result_from == 'google') { html += "<li class=\"result_lists\"><p>Address search by <strong>Google API</strong>.</p></li>"; }
	html += '</ul>';
...
function showLocation(address) {
	result_from = '';
	if(naver_api_check == 1) {
		if(!address) return;
		var params = new Array();
		params['component'] = "soo_google_map";
		...
		geocoder.geocode({'address': address}, function(a,b) {addAddressToMap(a, b); });
	}
}
...
function address_adder(a,b,address,results) {
	var result_cont = new Array(), response = new Array();

	if (b != google.maps.GeocoderStatus.OK) {
		result_cont = new Array();
	} else {
		result_cont = a;
		response = a;
	}

	var j=result_cont.length;

	for(var i=0;i<results.length;i++) {
		if(results[i].formatted_address || results[i].formatted_address != null) {
			response[j] = { from: results[i].result_from, formatted_address: results[i].formatted_address, geometry: {location : new google.maps.LatLng(results[i].geometry.lat, results[i].geometry.lng) } }
			j++;
		}
	}
